            ...
            with ui.tab_panels(tabs, value=t_lcn).classes("w-full"):
                ...
                with ui.tab_panel(t_lcn):
                    ...
                    with ui.column().classes("w-full gap-4 pt-4"):
                        ...
                        lcn_port = ui.number(
                            "Port", value=d_lcn.get("port", 4114), min=1, max=65535
                        ).classes("w-full")
                        ...
                        sk_num_tries = ui.number(
                            "SK num tries", value=d_lcn.get("sk_num_tries", 0), min=0
                        ).classes("w-full")
                ...
                with ui.tab_panel(t_mqtt):
                    ...
                    with ui.column().classes("w-full gap-4 pt-4"):
                        base_topic = ui.input(
                            "Base topic", value=d_mqtt.get("base_topic", "lcn2mqtt")
                        ).classes("w-full")
                        mqtt_host = ui.input(
                            "Host", value=d_mqtt.get("host", "")
                        ).classes("w-full")
                        mqtt_port = ui.number(
                            "Port", value=d_mqtt.get("port", 1883), min=1, max=65535
                        ).classes("w-full")
