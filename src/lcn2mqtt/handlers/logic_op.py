...
async def handle_get_command(
    ...
    for logic_op_idx in logic_ops:
        logic_op_state = result_input.states_logic_ops[logic_op_idx]
        bridge.publish(
            module.prefix,
            MqttMessage(
                f"logic_op/{logic_op_idx + 1}/state", logic_op_state.name.lower()
            ),
        )
